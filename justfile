...
a2 := ""
# a2.4.0: arm the gated cheap-mass call-site repoint inside 3a6b80 (oracle build + difftrace=1 for the DTWORLD gate; default OFF, orthogonal to a2/pfire). =1 3ac530 pass-through identity foothold; =2 redirect to the C DynamicTransform. Override: just a2body=1 difftrace=1 launch-foc-desktop
a2body := ""
...
pfirescan := ""
# B3.3.5 (§8.33, Fork B step 3): redirect 383f70's draw to a per-candidate substream (expect lmiss>0, wmiss/smiss=0; needs pfirescan=1 difftrace=1). Override: just difftrace=1 pfirescan=1 pfirescanss=1 launch-foc-desktop
pfirescanss := ""
...
pfirepool := ""
# THE FLIP — gated apply-side DRIVE (the validated sim/ writes live projectile state). Needs pfire=3 difftrace=1.
#   =1 (§8.65) bit-exact identity: overwrite the live rec's damage/lifetime/vis/muzzle_speed with the sim's
#      firing_build_projectile_init output (behavioral no-op; gated by DTWA-B3 DTB3SUM dmg/life/vis all-ok).
#   =2 (§8.67) +geometry direction: drive the body's dispersion (S[0..2]) via the sim's firing_apply_spread on a
#      per-fire substream in place of 381dc0 (same cone §8.66, substream sample §8.42; validated in-cone spr_oob=0).
#   =3 (§8.68) +geometry position: drive the muzzle-cone (spawn-pos basis S[8/9/6]) via firing_select_muzzle_point
...
sim-test:
  ...
  nix develop --command g++ -std=c++17 -O2 -Wall -Wextra -Isim \
    sim/tests/targeting_aim_test.cpp sim/targeting_aim.cpp -o /tmp/eaw_targeting_aim_test
  /tmp/eaw_targeting_aim_test
  nix develop --command g++ -std=c++17 -O2 -Wall -Wextra -Isim \
    sim/tests/geom_distance_test.cpp sim/geom_distance.cpp -o /tmp/eaw_geom_distance_test
  /tmp/eaw_geom_distance_test
  nix develop --command g++ -std=c++17 -O2 -Wall -Wextra -Isim \
  ...
  nix develop --command g++ -std=c++17 -O2 -Wall -Wextra -Isim \
    sim/tests/firing_aimpoint_test.cpp sim/firing_aimpoint.cpp sim/sim_rng.cpp -o /tmp/eaw_firing_aimpoint_test
  /tmp/eaw_firing_aimpoint_test
  nix develop --command g++ -std=c++17 -O2 -Wall -Wextra -Isim \
  ...
  nix develop --command g++ -std=c++17 -O2 -Wall -Wextra -Isim \
    sim/tests/fire_control_test.cpp sim/fire_control.cpp sim/firing_aimpoint.cpp sim/firing_intercept.cpp sim/firing_spawn.cpp sim/sim_parallel.cpp sim/sim_rng.cpp -o /tmp/eaw_fire_control_test
  /tmp/eaw_fire_control_test
  nix develop --command g++ -std=c++17 -O2 -Wall -Wextra -Isim \
  ...
  nix develop --command g++ -std=c++17 -O2 -Wall -Wextra -Isim \
    sim/tests/spline_mover_test.cpp sim/spline_mover.cpp -o /tmp/eaw_spline_mover_test
