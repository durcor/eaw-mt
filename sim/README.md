...
| **locomotor integrators** | Starship `6236b0`+`6224b0` (+`4c2f70` `3a8f90` `623340` `4aaa40` `2bab90` `559250` `370f00`); SimpleSpace `626420` | ✅ Starship skeleton + integrator lifted; SimpleSpace dispatch skeleton lifted (its mover `625990` is a hook); data model + primitives shared. Deep per-type movers behind `LocomotorEnv`/`SimpleSpaceEnv` | reads/writes `entity+0x78`; **two families**: Starship = raw velocity `state+0x14`; SimpleSpace = unit-direction `+0x14` + speed `+0xec` (common space mover). bit-exact oracle needs each family's mover lifted |
