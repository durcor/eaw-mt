...
| command/event queue | `058570` `220ed0`/`e90`/`eb0` `240940` `1cacb0` (signal/slot); `2d5290` `OutgoingEventQueueClass` (sched. queue) | 🟡 FRAMED (`CommandSink`) + **both `387400` emitters wired & host-validated** | **Two channels**: (1) per-object signal/slot — each GameObject embeds a `SignalDispatcherClass` at `+0x38` (hash-map `sig_id→SignalListClass`, insertion-ordered slots; emit walks head→tail). (2) global scheduled-event queue `OutgoingEventQueueClass` (orders by `(fire_time,insertion)`). `387400` emits 2 signals, now **both lifted & wired to `sim::CommandSink`** (`sim/recording_command_sink.h` is the reusable Phase-A sink); the two are **mutually exclusive within a call** (the `cVar6` gate at line 221 / the `state_flag` return at line 203) — `387400` emits EITHER 0x20 OR (via the tail) 0x21, never both. Emission decisions are deterministic given unit inputs & slot-independent → recording `CommandSink` is a faithful Phase-A seam; dispatch fan-out (`240940`) is Phase-B, not yet lifted. Canonical Phase-B apply order = per-emission insertion order × GOM tick order. **✅ `acquire_opportunity_target` (lines 220–316, the `scan_opportunity_target` *caller*) → sig 0x21**: the 4 emission outcomes (keep-existing / emit-after-rescan / bound-not-fire-allowed→clear / emitter-bail) + the rescan-stamp comma side effect host-validated. **✅ `commit_ordered_fire` (lines 173–201) → sig 0x20** (parameterless FireOrderInProgress): the in_progress_flag 0→1 transition (ordered & regular branches, edge-only emit, not-committed clear) host-validated, incl. the distinguishing **flag-persists-on-emitter-bail** subtlety (0x20 sets the flag *before* resolving the emitter and bails via `goto`, so the transition survives an emit suppression — unlike 0x21's full-`return` bail) |
