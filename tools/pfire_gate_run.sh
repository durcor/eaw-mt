...
# Usage: pfire_gate_run.sh <tag> <geomss 0|1> <reorder 0|1>
set -u
TAG="$1"; GEOMSS="$2"; REORDER="$3"
...
just pfire=4 pfireshards=4 pfiregeomss="$GEOMSS" pfirereorder="$REORDER" difftrace=1 \
...
echo "=== gate cell $TAG (geomss=$GEOMSS reorder=$REORDER) ==="
cat "$OUT"
grep -E "PFIREGEOMSS|PFIREREORDER|geom_ss_fires|shard_maxfill|PFIRE-DRAIN|rank_down" "$LOG" 2>/dev/null | tail -4
