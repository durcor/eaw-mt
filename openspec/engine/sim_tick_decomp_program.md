...
- **(C-gap) The iteration set + order as data** — the driver visits objects in a specific order
  (registry order, per DIFFTRACE). This must be lifted to an **explicit ordered owned-object list** the
  `ShardScheduler` consumes — *characterized*, not reconstructed as loop code. DIFFTRACE already proves
  the set+order; this gap is turning that into the scheduler's partition input.
