...
//   applies them. Canonical order == (entity_id, per-entity emission seq): entity_id is the GOM tick
//   key (registry append index), seq is the monotonic counter of that entity's emissions this tick.
//   Because each entity is owned by exactly one shard, (entity_id, seq) is globally unique, so the
//   merge is a total deterministic sort — INDEPENDENT of which shard produced an op or the wall-clock
//   order shards finished. That independence is the whole correctness argument.
//
...
struct BufferedOp {
    uint32_t     entity_id = 0;     // GOM tick key (registry append index of the emitter)
...
struct ShardBuffer : CommandSink {
    ...
    // Open the emission scope for one owned object (Phase A). Resets the per-entity emission counter.
    void begin_entity(uint32_t entity_id) {
