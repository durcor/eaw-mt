...
| command/event queue | `058570` `220ed0`/`e90`/`eb0` `240940` `1cacb0` (signal/slot); `2d5290` `OutgoingEventQueueClass` (sched. queue) | 🟡 FRAMED (`CommandSink`) + **first emitter wired & host-validated** | **Two channels**: (1) per-object signal/slot — each GameObject embeds a `SignalDispatcherClass` at `+0x38` (hash-map `sig_id→SignalListClass`, insertion-ordered slots; emit walks head→tail). (2) global scheduled-event queue `OutgoingEventQueueClass` (orders by `(fire_time,insertion)`). `387400` emits 2 signals: `0x20` fire-in-progress (parameterless), `0x21` `OpportunityTargetAcquired` (3-ptr payload). Emission decisions are deterministic given unit inputs & slot-independent → recording `CommandSink` is a faithful Phase-A seam; dispatch fan-out (`240940`) is Phase-B, not yet lifted. Canonical Phase-B apply order = per-emission insertion order × GOM tick order. **✅ `acquire_opportunity_target` (`387400` lines 220–316, the `scan_opportunity_target` *caller*) now lifted & wired to `sim::CommandSink`** (`sim/recording_command_sink.h` is the reusable Phase-A sink): the 4 sig-0x21 emission outcomes (keep-existing / emit-after-rescan / bound-not-fire-allowed→clear / emitter-bail) + the rescan-stamp comma side effect are host-validated. sig 0x20 (ordered-fire `in_progress` transition, lines 173–194) stays env-modelled |
