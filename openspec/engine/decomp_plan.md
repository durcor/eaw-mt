...
|---|---|---|---|
| `SimpleSpaceLocomotorBehaviorClass` (+Walk/Fighter/Fleet/Starship…) | `0x626420` | **IN** | 🟡 **DECODED (2026-06-04) — the mover core `vfunc_59` is a liftable cubic-spline path interpolator; the `vfunc_6` wrapper is a deferred-seam FSM.** `vfunc_6` (1484 B): base `LocomotorCommonClass::vfunc_6`, then predicate vfunc `+0x210` → dispatch by order-state (`order+0x48`) to 4 per-state movers — **4**=hyperspace-in `FUN_140626c80` (2316 B: velocity→lane projection / closest-approach remap, **RNG `FUN_1401ffb40`**, frame-counter FSM, GOM, **SFX `FUN_1402d5320`**, debug printf); **5**=jump-prep `FUN_140627590` (normalize+project, writes **global scratch** `DAT_140b27bbc/c0`, → state 4); **0x28**=spin/spawn `FUN_1405cd680` (sin/cos-rotated velocity + integrate, **SFX**, counter dec); **0x2c**=death-drift `FUN_1406269f0` (normalize+integrate, **SFX**, render-state) — all 4 inseparable from GOM/RNG/SFX/global-scratch = **deferred Phase-B seam**. Else-branch: states **0**/**0x13** (align-into-formation; Moving state calls the mover virtual `+0x1d8`)/**0x16** (clamp-toward-zero decel of speed `+0x84`); tail = damage-flicker render glow + hyperspace-ready booleans (`FUN_1403a7e30/cf0`). **The actual mover = `vfunc_59` `0x625990` (2354 B) = a cubic-spline (Hermite) waypoint-path interpolator** [LIFT TARGET, not yet lifted; flagged 2026-05-30 as "the bit-exact oracle step"]: advance keyframe index over a waypoint array (`order+0x18`, `FUN_14054fc00`), `t=clamp((time−t0)/(t1−t0),0,1)`, then **(a)** equal-endpoint → orientation-only interp reusing **`FUN_14020acd0`=aim (already lifted `targeting_aim`)** + euler-delta-wrap `FUN_14020b100` + **`FUN_14020b6d0`=wrap_180 (lifted)** + euler-add-wrap `FUN_14020b070`; **(b)** else → full **cubic position+derivative** via a 4×4 basis matrix `DAT_140b2f1f0..22c` (+`t²`,`t³`, tangents, normalize `FUN_140139800`). Helpers `139800`/`20b100`/`20b070` are small & pure. Core is separable (engine-state touches = waypoint fetch `54fc00` input, iterator advance `5c4920`/`5c5910`, writeback `624c10` output) — same I/O-boundary pattern as `targeting_aim`. ✅ **`vfunc_59` core LIFTED 2026-06-04 → `sim/spline_mover.{h,cpp}` (host-validated, 7 structural groups PASS).** Basis matrix `DAT_140b2f1f0` (runtime-built by `FUN_14042b9f0` = inverse `FUN_1402324a0` of the cubic constraint matrix `[f(0);f(1);f'(0);f'(1)]`) extracted = **exact integer standard Hermite basis**; shared by 3 locomotor-family `vfunc_59` (625990/63c340/641010). Core = 2D XY Hermite: `m=normalize(heading)*speed*dT`, `pos=((b*t+a)+c*t2)+d*t3`, `vel=((2c*t+b)+3d*t2)/dT`, `speed=|vel_xy|`, Z passthrough; orientation reuses lifted `targeting_aim_angles`. ✅ **In-game DTSPL2 oracle = bit-exact PASS (2026-06-04).** The first capture (folded into `b3a76b0_hook`) read the locomotor coordinator `coord+0xa8` and its `+0x18` node array — the **wrong object** — so every Moving-state ship hit the empty-path branch (`base0`-for-everyone, `cnt2=0`). The decompile shows the path/order object is `vfunc_59`'s **`param_2`** (nodes via engine indexer `FUN_14054fc00` at `param_2+0x18`, segment idx `+0x60`, sub-mode `+0x48`, exhaustion predicate `FUN_1405c5910`). Re-pointed to a real **`0x625990` entry detour** (14-byte clean-boundary trampoline): run the original, recompute the settled segment in `sim/spline_mover` terms, compare raw bits to `*param_7`, scoring only the clean cubic-Hermite branch (mode≠5, not exhausted, dt>0, distinct endpoints). Live multi-battle result: **pass=1,374,051 fail=0 cmp=1,374,051** over `calls=1,609,276` (mode5=233150 / ended=2074 / degen=1 gated out), **DTSPL2MISS=0** — the Hermite mover is the *dominant* ship-movement path (1.6M calls) and `sim/spline_mover` reproduces it bit-exactly in-game. Samples archived in `eaw-mt.log.dtspl2-pass`. **Also identified (NOT lifted): `FUN_1407d8310` = self-contained 3D value-noise** (perm `DAT_140a45d20`, value `DAT_140a45f20`, smoothstep `(3−2t)·t²`, trilinear) — **single-caller** (damage-flicker `0x626885`), Class 3 presentation glow, cold → low-value deferred. **Closes the standalone IN-behavior decode slice.** |
...
| `ShieldBehaviorClass` | `0x5495e0` | **IN** | ⛔ **CHARACTERIZED — NO numeric core (2026-06-04)**: NOT shield regen — it is an **ion-storm detector**. Sphere-scan `FUN_1405d7c00` over nearby objects; on the first object carrying the ion-storm flag (`template+0x298` byte `+0x56`) stamp the entry tick `lVar1+0x100=now` (logs "entered Ion Storm"), else `-1`. Same orchestration-seam class as #4/#6 (the distance gate lives INSIDE the sphere query — nothing thin to lift). Skipped as a lift target |
...
| `IonStunEffectBehaviorClass` | `0x62bba0` | **IN** | ⛔ **CHARACTERIZED — NO numeric core (2026-06-04)**: a **thin presentation/delegation** behavior (73 bytes, no float math). Body: (1) base `LocomotorCommonClass::vfunc_6`; (2) if the entity has a render object (`entity+0x2a0`), call the **render sub-state setter** `FUN_140265ae0(renderObj, 2)` — objdump shows it walks the visual's component linked-list (sentinel vtbl `0x140a12370`) and for the matching component sets animation-state slot `2` (the ion-stun visual) via dispatch-table `0x140a15d20[2]`→`0x140266790`, storing `(flag^1)+1` into `renderObj+0x20+idx*4`; (3) virtual dispatch `vfunc+0xc0` on self; (4) returns the behavior-class tag `0x650000`. No spring/geometry/RNG — nothing bit-matchable to lift. **Class 3 presentation** (writes only the render object's anim-state). Skipped as a lift target (same outcome as Shield #5495e0) |
...
| `SelectBehaviorClass` | `0x3c2310` | **out** | ✅ **LIFTED (host-validated)** — the UI selection-marker behavior (ring/brackets/health-bar decals at `entity+0xb0`, +0x10/+0x18/+0x20/+0x28/+0x30). Body is pure PRESENTATION orchestration (env): name label `3c2440`, decal create `3c19c0` / destroy `3c1d80` / per-frame place `3c2710` at `entity+0x78`. Its ONLY bit-matchable core is the tail **selection-pulse spring** on the behavior object (`param_1+0x44` value / `+0x48` equilibrium / `+0x4c` velocity / `+0x54` freq), which disassembles **INSTRUCTION-IDENTICALLY** to Nebula #5's STAGE-1 spring (same SSE seq, same .rdata consts 2.0/0.48/0.235/1.0, same dt) → factored the shared routine into `sim/damped_spring.{h,cpp}`; Select & Nebula both delegate. **⚠️ asm grouping:** binary denom = `quad + (wdt+1.0) + cubic` (NOT the flattened-decompile left-assoc `((quad+wdt)+1.0)+cubic` — diverges ~9% over a wide range; the original nebula.cpp's left-assoc passed DTNEB only because Nebula's small-wdt data makes the two coincide). **Threading: Class 3 presentation, off-lockstep, Phase-A-safe** (spring writes own fields; orchestration writes render/decal objects, no sim state/RNG/GOM). Host: `damped_spring_test` ground-truths the lift vs an inline-asm transcription of the exact SSE sequence **4,000,000/4,000,000 bit-exact** (grouping divergence 9.24% confirms the parenthesization is load-bearing); `select_marker_test` **2,000,000/2,000,000**. In-game bit-exactness **INHERITED** from DTNEB (614503/614503 on the identical instructions) — no separate DTSELECT capture required (same shared-primitive-inherits-validation pattern as #7 face_from_yaw ← #8 z-bob) |
| `HideWhenFoggedBehaviorClass` | `0x53ddc0` | **out** | ✅ **LIFTED (host-validated)** — the fog/detection **alpha-fade** behavior (smooth fade-in/out as a unit enters/leaves the player's sensor coverage), closing the OUT presentation slice **2/2**. Body is PRESENTATION orchestration (env): a 30-tick update-coalescing throttle at `param_1+0x58` (`1404ad350`=interval-elapsed? / `1404ad380`=arm 0x1e, settled-skip uses `fabs 1407760bc < DAT_140815de4`); a visibility decision against the shroud manager `DAT_140b15418` (`14039b950`/`14035f470`/`14039a450`/`14039b140`) that picks the fade target 0/1 and latches `+0x54`/notifies reveal-`1404eb020`/hide-`1404eacf0`; a render toggle `140264c80(entity+0x2a0, visible)` + shroud vtbl `+0x160`/`+0x168` on the alpha crossing. Its ONLY bit-matchable core is the **fog-fade spring** on the behavior object (`param_1+0x40` value / `+0x44` equilibrium / `+0x48` velocity / `+0x50` freq), the **THIRD caller** of the shared engine spring: objdump of `0x53ddc0` shows the **same SSE sequence**, same five `.rdata` consts (2.0/0.48/0.235/1.0 + dt), and the **same load-bearing denom grouping** `quad + (wdt+1.0) + cubic` (verified addss order wdt+1.0→+quad→+cubic→divss); state offsets shifted **−4** vs Select but the relative `{value,eq,vel}`+freq@+0x10 layout is identical → delegates to `sim/damped_spring.{h,cpp}`. **Instant-reveal snap** (`param_2->0x298+0x88`): override value=eq, vel=0 (no fade) — env-gated, not spring math. **Threading: Class 3 presentation, off-lockstep, Phase-A-safe** (spring writes own fields; orchestration writes render/visibility/shroud objects, no sim state/RNG/GOM). Host: `hide_when_fogged_test` **2,000,000/2,000,000 bit-exact** vs the shared primitive (itself inline-asm ground-truthed 4M/4M). In-game bit-exactness **INHERITED** from DTNEB (614503/614503 on the identical instructions) — no separate capture required (shared-primitive-inherits-validation pattern, now its third instance) |
...
the engine's **Force force-grip / telekinesis** effect — the target-side behavior driven by the
`<Type>FORCE_TELEKINESIS</Type>` ability (Palpatine / Luke Darkside / Tremayne / Brakiss lifting a single
enemy *vehicle*). It is a **3-mode lifecycle state machine** over a per-entity telekinesis slot
(`slot = *(entity+0x160)`; mode int at `slot+0x8`): mode 1 **GRAB** (interp toward the grip pose, on
`t>=1` advance to mode 2), mode 2 **HOLD** (`63f730`: spin-ramp + periodic 10%/0.2s damage + GOM spawn —
the XML `Damage_Percentage 0.10` / `Damage_Interval 0.2`), mode 3 **RELEASE** (`63f470`: interp back, on
`t>=1` zero the slot + re-enable abilities + GOM); else Idle; `slot==0` → distinct NoOp `0x80650001`.
Like Targeting #7, **part 1 lifts the shared interp-timeline core + mode dispatch** and DEFERS the two
heavy sub-bodies (HOLD damage/spawn + RELEASE ability-re-enable/GOM — the cross-boundary Phase-B seam).
Modes 1 & 3 share one timeline:
  - `now = (float)sim_clock / (float)hz` (`DAT_140b0a320 / DAT_140b0a340`)
  - `t = max((now - start)/duration, 0)` (clamp-low only; `start = slot+0x24`, `dur = _DAT_140b15ac4`)
  - `complete = (1.0 <= t)` (inclusive)
  GRAB lerps `entity+0x80` toward `DAT_140b15ac0 + slot+0x14`; RELEASE toward `slot+0x14` directly. On
  ...
  *only* writes to `slot+0x8` in either body, so a mode change across the tick == a completion edge.
  Mode 1 GRAB is Phase-A-safe (writes only its own slot + own transform); modes 2/3 completion
  side-effects are the deferred seam. 6 host cases (dispatch / now / t clamp+complete / lerp / angle gate /
  target) all PASS.
  **DTTK in-game oracle** (`hooks/winmm_proxy.c`, `EAW_DIFFTRACE=1`): inline trampoline on `0x63f210`
  (34-byte position-independent prologue, cut before the first `E8` base-vfunc_6 call at +0x22; arg
  count 3). Per telekinesis-entity-tick on an interp mode it reads the slot fields + config globals
  BEFORE the trampoline, computes the predicted timeline, runs the real call, then validates three things
  in-process (bit-exact `neb_biteq`): **(1) completion biconditional** `pred_comp == act_comp`
  (`mode_after != mode`) — the lifted `t>=1` edge fires exactly when the binary changes `slot+0x8`;
  **(2) rebase bit-exact** on every completion edge `slot+0x24_after == now`; **(3) interp lerp bit-exact**
  on every non-completion tick `entity+0x80 == lerp(slot+0x30, to, t)` — the continuously-observable
  quantity (Nebula-class). Emits `DTTK` on completion edges / mismatches / first-40 + periodic
  `DTTKSURVEY` totals. `tools/analyze_telekinesis_oracle.py` aggregates and re-derives the booleans'
  consistency. **Result over a land battle with the Emperor force-gripping enemy vehicles (8 distinct
  entities; survey total 144245 behavior-ticks, `grab/hold/rel/idle = 353/539/52/143301`):**
  - **completion biconditional 405/405** (`bicond_bad = 0`) across BOTH interp modes — GRAB 1→2 *and*
    RELEASE 3→0 exercised (17 completion edges).
  - **rebase bit-exact 17/17** (`rebase_bad = 0`) — `slot+0x24 := now` reproduced exactly on every edge.
  - **interp lerp bit-exact 388/388** (`interp_bad = 0`) — 388 continuously-observed `entity+0x80` set-
    position Z values matched `lerp(slot+0x30, to, t)` bit-for-bit, with the mode-specific `to`
    (`angle_base + slot+0x14` for GRAB, `slot+0x14` for RELEASE).
  - HOLD (mode 2, 539 ticks) was exercised as a *passthrough* — its sub-body (`63f730`) runs inside the
    trampoline but is not part of part-1's lifted core (the deferred damage/spawn seam). Capture saved
    `logs/dttk_oracle_pass.log`. Captured with the `EAW_ORACLE` build.

**🟡 Behavior #8 (part 2) — `TelekinesisTargetBehaviorClass` HOLD body (`FUN_14063f730`, mode 2) LIFTED
+ DTTK ORACLE PASS.** Once GRAB completes (`slot+0x8 := 2`) the entity enters HOLD; each tick the body
does two independent things, both lifted into `sim/telekinesis_target.{h,cpp}` as the deterministic core:
  - **POSE (spin + z-bob):** phase angle `= spin_t·omega` where `spin_t = max((now−start)/spin_period, 0)`
    (`spin_period = _DAT_140b15ac8`, `omega = DAT_140819b3c`). The orientation spins on a `cos/sin` circle
    of radius `DAT_140b15ad0` ramped in over `DAT_140b15acc`; the **z-position bobs** by
    `pos_z = slot+0x14 + (sin(angle)·bob_amp + height_offset)` (`bob_amp = _DAT_140b15ad4`,
    `height_offset = DAT_140b15ac0` — the SAME grip-height const GRAB adds; the z-bob has **no** ramp).
    `sin` uses the engine's own `FUN_140776650` but only when the angle clears a small-angle guard
    (exp field `> 0x1d`), else `sin → angle`. `pos_z` lands in `entity+0x80` — the SAME observable GRAB's
    part-1 lerp drives, so the oracle reuses that channel.
  - **DAMAGE (self-clocked periodic):** a deadline at `slot+0x48` — **due iff** `(slot+0x48 <= now &&
    slot+0x48 != 0)`; on fire it applies `slot+0x40` damage to **SELF** (`FUN_1403a9e30`, like
    AsteroidFieldDamage #6 the gameplay write is self-directed) and advances `slot+0x48 := interval
    (slot+0x44) + slot+0x48`, then fires the event dispatch `FUN_1402d5320` (the ONLY cross-boundary part
    — **DEFERRED**). The cadence is a pure deterministic clock.

  **DTTK HOLD oracle** (extends the part-1 hook; mode-2 ticks emit `DTTKHOLD` lines + `hold_*` survey
  totals): **(4) z-bob pose bit-exact** `entity+0x80 == slot+0x14 + (sin(angle)·bob_amp + height_offset)`
  (the in-process hook calls the engine's own `FUN_140776650` for bit-exactness); **(5) damage-deadline
  rebase bit-exact** `slot+0x48_after == interval + slot+0x48` on fires. **Result over a land battle with
  the Emperor gripping enemy vehicles (survey total 613k+ behavior-ticks, 174 HOLD ticks, 33 damage
  fires): z-bob pose 174/174 (`hold_pose_bad = 0`), damage rebase 174/174 (`hold_sched_bad = 0`).**
  - **Bug found & fixed by the bit-exact oracle:** the first capture showed pose 95/**11** — 11 sub-ULP
    mismatches clustered at large `|sin·bob|`. Cause: the Ghidra decompile flattens line 57 to a
    left-assoc `slot_z + sin·bob + off`, but the actual codegen (`63f730 @0x63f8d3–0x63f905`) groups it
