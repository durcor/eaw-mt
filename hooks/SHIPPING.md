...
| `EAW_DIFFTRACE=1` | Profile build only — emit the per-tick sim fingerprint (`DIFFTRACE` lines: movement-state machine + each serviced GameObject's world position `+0x78`; plus a `DTPOS` x/y/z sample and a `DTVEL` line carrying the sampled ship's locomotor velocity `state+0x14/18/1c` + state `+0x48`) used as the decomp differential-test oracle. Capture via `just difftrace=1 launch-foc-desktop`; analyze the locomotor position-integration with `tools/analyze_loco_oracle.py`. |
