...
| hardpoint fire-control | `a76b0` `387f50` `540070` `387010` (+ `396df0`/`396070`/`387400` hooks) | 🟡 fire-budget distribution + consume + per-mount dispatcher skeleton lifted; ship scalars + target-acq (`387400`) behind `HardpointEnv` | **`total_w` cancels** → `share_i = budget_i·(capacity−base)`; the game-speed weight scale (`540070`) is irrelevant to every share. DIFFTRACE already folds budget `+0x28` / weight `+0x58`; DTFIRE capture = the precise oracle (pending). target-acq RNG is the deferred sub-lift |
